...
std::string VideoDoViCodecString() {
  ...
  profile = CServiceBroker::GetDataCacheCore().GetVideoDoViStreamInfo().dovi.dv_profile;
  std::string video_type = HdrTypeToString(CServiceBroker::GetDataCacheCore().GetVideoSourceHdrType());
  if ((profile == 0) && (video_type.compare("Dolby Vision") == 0)) { profile = 4; fourCC.assign("----"); }
...
bool CPlayerGUIInfo::GetLabel(std::string& value, const CFileItem *item, int contextWindow, const CGUIInfo &info, std::string *fallback) const
{
  switch (info.m_info)
  {
    ...
    case PLAYER_PROCESS_VIDEO_DOVI_PROFILE:
    {
      ...
      if ((profile == 4) && (value.compare("0") == 0))
        value.assign("4");
      return true;
    }
